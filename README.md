# WebDAV File Manager (个人网盘与 WebDAV 服务器)
...
- **基本文件操作**: 支持拖拽文件上传、直接点击上传、普通下载、复制、移动、删除、重命名，还支持新建空白文件或直接创建并写入文本。
...
- **全功能个人网盘 + WebDAV 协议**: 既可通过精美的网页端进行全功能的文件管理（即开即用的私人云盘），又原生支持 WebDAV 协议，可使用 Windows 资源管理器、macOS Finder、RaiDrive 或其他第三方客户端将其直接挂载为本地磁盘。
